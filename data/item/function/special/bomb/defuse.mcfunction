...
tellraw @a[tag=role.Murder] [{"color":"red", "text":"[ラブスペ人狼] "}, {"text":"仕掛けた一人から爆弾に気づかれ、解除されてしまいました"}]

execute if entity @a[tag=player.Bombed] run return 0

clear @a[tag=role.Murder] redstone_torch
tellraw @a[tag=role.Murder] [{"color":"red", "text":"[ラブスペ人狼] "}, {"text":"仕掛けた市民全員が爆弾に気づき、解除したようだ"}]
