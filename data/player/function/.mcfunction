...
execute if entity @s[tag=!BadGuys, team=!detective] at @s if predicate player:is_sneaking_on_ground if entity @e[tag=Escape, distance=..2, type=armor_stand, limit=1] run return run function player:sneak/
...
execute at @s if entity @s[team=maniac, tag=SneakingWithBow] run function player:sneak/maniac/cancel
execute at @s unless predicate player:is_sneaking_on_ground if entity @s[tag=Escaping] run function player:sneak/cancel
