# villager
tag @a remove adv.burp
tag @a remove adv.more_burp
tag @a remove adv.trash
tag @a remove adv.lot_trashes
tag @a remove adv.totem
tag @a remove adv.third_totem
tag @a remove adv.used_ironhorse
tag @a remove adv.oopsy_ironhorse
tag @a remove adv.ironhorse.bow
tag @a remove adv.ironhorse.knife
tag @a remove adv.sign_early
tag @a remove adv.not_attack
tag @a remove adv.not_key_lost
tag @a remove adv.not_running
tag @a remove adv.not_trade

# murder
tag @a remove adv.lantern_witness
tag @a remove adv.mur.uses_bandage
tag @a remove adv.mur.win
tag @a remove adv.mur.perfect
tag @a remove adv.next_detective
tag @a remove adv.first_kill_witness

# detective
tag @a remove adv.det.nothing
tag @a remove adv.defeat_everyone
tag @a remove adv.defeat_murder
tag @a remove adv.defeat_villager
tag @a remove adv.det.duplicate
tag @a remove adv.det.rta

# witness
tag @a remove adv.noph2escape
tag @a remove adv.ph2escape
tag @a remove adv.wit.escape_with_lantern
tag @a remove adv.wit.invis
tag @a remove adv.actual_invis
tag @a remove adv.wit.more_invis

# secret
tag @a remove adv.secret3.escape

scoreboard players reset * adv.trashcount
scoreboard players reset * adv.totemcount
scoreboard players reset * adv.mur.kills
scoreboard players reset * adv.arrowcount
scoreboard players reset * adv.stopwatch
scoreboard players reset * adv.drink_invis
scoreboard players reset * adv.secret1
scoreboard players reset * adv.secret2
scoreboard players reset * adv.secret3
